					...
					/* array in the code)		*/
{
    ...
    pptr->vmemlist = NULL;
	for (i = 0; i < 8 ; i++) {
		pptr->bs_used[i].bs_status = BSM_UNMAPPED;
		for (j=0; j < NPROC ; j++){
         pptr->bs_used[i].bs_pid[j] = 0;
		}
		pptr->bs_used[i].bs_vpno = -1;
		pptr->bs_used[i].bs_npages = 0;
		pptr->bs_used[i].bs_sem = -1;
        pptr->bs_used[i].bs_vheap = 0;
        }
		/* Bottom of stack */
	*saddr = MAGIC;
